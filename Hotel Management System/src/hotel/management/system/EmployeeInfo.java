...
public class EmployeeInfo extends JFrame implements ActionListener{
	...
	JButton b1, b2;
	int i = 0, j = 0;
	String s[][] = new String[20][8];
	String headings[] = {"Name, Age", "Gender", "Department", "Salary", "Phone No.", "Aadhar", "Email"};
	...
	public void actionPerformed(ActionEvent ae) {
		if(ae.getSource() == b1) {
			...
			try {
				...
				while(rs.next()) {
					    s[i][j++]=rs.getString("name");
		                s[i][j++]=rs.getString("age");
		                s[i][j++]=rs.getString(3);
		                s[i][j++]=rs.getString(4);
		                s[i][j++]=rs.getString(4);
		                s[i][j++]=rs.getString(5);
		                s[i][j++]=rs.getString(6);
		                i++;
		                j=0;
				}
				t1 = new JTable(s,headings);
